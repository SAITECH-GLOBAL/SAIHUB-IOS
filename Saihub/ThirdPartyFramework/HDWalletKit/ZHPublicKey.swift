...
public struct ZHPublicKey {
    ...
    private let privateKey: ZHPrivateKey?
    ...
    init(privateKey: ZHPrivateKey, chainCode: Data, network: Network, depth: UInt8, fingerprint: UInt32, index: UInt32) {
    ...
    public init(ypub: String, network: Network) throws {
        var decoded = ypub.base58CheckDecodedData
        if (decoded != nil)
        {
            decoded = Data(bytes: decoded!.bytes[4...(decoded!.count-1)])
            var bytePrefix:[UInt8] = [04,0x88,0xb2,0x1e]
            var prefixData = Data(bytes: bytePrefix, count: 4)
            let xpubData: Data = Data(bytes: prefixData + decoded!)
            let xpub = xpubData.bytes.base58CheckEncodedString
            try! self.init(xpub:xpub , network: network)
        }else
        {
            throw Errors.base58DecodingFailed
        }
    }
    public init(zpub: String, network: Network) throws {
        var decoded = zpub.base58CheckDecodedData
        if (decoded != nil)
        {
            decoded = Data(bytes: decoded!.bytes[4...(decoded!.count-1)])
            let bytePrefix:[UInt8] = [04,0x88,0xb2,0x1e]
            let prefixData = Data(bytes: bytePrefix, count: 4)
            let xpubData: Data = Data(bytes: prefixData + decoded!)
            let xpub = xpubData.bytes.base58CheckEncodedString
            try! self.init(xpub:xpub , network: network)
        }else
    ...
    private func derived(at index: UInt32) throws -> ZHPublicKey {
        ...
        return ZHPublicKey(raw: publicKey, chainCode: derivedChainCode, network: self.network, depth: self.depth + 1, fingerprint: self.fingerprint, index: index)
    }
    
    public func derived(path: UInt32,index: UInt32) throws -> ZHPublicKey {
        guard let derived = try? self.derived(at: path).derived(at: index) else {
