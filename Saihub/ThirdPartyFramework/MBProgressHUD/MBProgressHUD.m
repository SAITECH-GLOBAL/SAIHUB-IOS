...
- (void)setupViews {
    ...
    bezelView.backgroundColor = rgba(0, 0, 0, 0.7);
    bezelView.alpha = 0.f;
...
//    detailsLabel.font = [UIFont boldSystemFontOfSize:MBDefaultDetailsLabelFontSize];
    detailsLabel.font = [UIFont systemFontOfSize:MBDefaultDetailsLabelFontSize];
...
- (void)updateConstraints {
    ...
    NSDictionary *metrics = @{@"margin": @(margin + 29)};
    NSDictionary *metricsSub = @{@"margin": @(margin)};
    NSDictionary *metricsV = @{@"margin": @(margin + 15),@"marginBottom": @(margin - 14)};
    ...
    [subviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
        ...
        [bezelConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"|-(>=margin)-[view]-(>=margin)-|" options:0 metrics:metricsSub views:NSDictionaryOfVariableBindings(view)]];
        [bezelConstraints addObjectsFromArray:[NSLayoutConstraint constraintsWithVisualFormat:@"V:|-(>=margin)-[view]-(>=marginBottom)-|" options:0 metrics:metricsV views:NSDictionaryOfVariableBindings(view)]];
...
- (void)updateForCurrentOrientationAnimated:(BOOL)animated {
    ...
    if (self.superview) {
        self.frame = self.superview.bounds;//全屏展示
//        self.frame = CGRectMake(0,44 + kStatusBarHeight, kWIDTH, kHEIGHT - (44 + kStatusBarHeight));//空出顶部导航栏展示
    }
