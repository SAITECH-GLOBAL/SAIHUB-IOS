...
#define kBTHDAccountLookAheadSize (50)
...
- (instancetype)initWithMnemonicSeed:(NSData *)mnemonicSeed withPassphrase:(NSString *)passphrase password:(NSString *)password fromXRandom:(BOOL)fromXRandom andGenerationCallback:(void (^)(CGFloat progres))callback {
    self = [self initWithMnemonicSeed:mnemonicSeed withPassphrase:passphrase btBip39:nil password:password fromXRandom:fromXRandom syncedComplete:YES andGenerationCallback:callback];
    return self;
}

- (instancetype)initWithMnemonicSeed:(NSData *)mnemonicSeed withPassphrase:(NSString *)passphrase btBip39:(BTBIP39 *)bip39 password:(NSString *)password fromXRandom:(BOOL)fromXRandom syncedComplete:(BOOL)isSyncedComplete andGenerationCallback:(void (^)(CGFloat progres))callback {
    self = [super init];
    if (self) {
        self.hdAccountId = -1;
        self.mnemonicSeed = mnemonicSeed;
        self.hdSeed = [BTHDAccount seedFromMnemonic:self.mnemonicSeed withPassphrase:passphrase btBip39:bip39];
...
- (instancetype)initWithEncryptedMnemonicSeed:(BTEncryptData *)encryptedMnemonicSeed withPassphrase:(NSString *)passphrase btBip39:(BTBIP39 *)bip39 password:(NSString *)password syncedComplete:(BOOL)isSyncedComplete andGenerationCallback:(void (^)(CGFloat progres))callback {
    self = [super init];
    if (self) {
        self.hdAccountId = -1;
...
//        @throw [[DuplicatedHDAccountException alloc] initWithName:@"DuplicatedHDAccountException" reason:@"DuplicatedHDAccountException" userInfo:nil];
    ...
    NSArray *subOutsExternal = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccountId:self.hdAccountId pathType:EXTERNAL_BIP49_PATH];
...
- (BTTx *)newTxToAddresses:(NSArray *)toAddresses withAmounts:(NSArray *)amounts dynamicFeeBase:(uint64_t)dynamicFeeBase andError:(NSError **)error andChangeAddress:(NSString *)changeAddress coin:(Coin)coin {
    ...
    if (error && !tx) {
        for (BTOut *outItem in outs) {
            [[BTTxProvider instance] remove:outItem.txHash];
        }
        return nil;
    }
    tx.coin = coin;
...
- (BTTx *)newTxToAddress:(NSString *)toAddress withAmount:(uint64_t)amount pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error {
    return [self newTxToAddresses:@[toAddress] withAmounts:@[@(amount)] andChangeAddress:[self getNewChangeAddressForPathType:pathType] pathType:pathType dynamicFeeBase:dynamicFeeBase password:password andError:error coin:BTC];
}
- (BTTx *)newTxForVirtualTradingToAddresses:(NSString *)toAddress withAmount:(uint64_t)amount pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error{
    return [self newTxForVirtualTradingToAddresses:@[toAddress] withAmounts:@[@(amount)] andChangeAddress:[self getNewChangeAddressForPathType:pathType] pathType:pathType dynamicFeeBase:dynamicFeeBase password:password andError:error coin:BTC];
}
- (BTTx *)newTxForVirtualTradingBc1ToAddresses:(NSString *)toAddress withAmount:(uint64_t)amount withOuts:(NSArray *)outs andChangeAddress:(NSString *)changeAddress pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error{
    return [self newTxForVirtualTradingBc1ToAddresses:@[toAddress] withAmounts:@[@(amount)] withOuts:outs andChangeAddress:changeAddress pathType:pathType dynamicFeeBase:dynamicFeeBase password:password andError:error coin:BTC];
}
...
- (BTTx *)newTxToAddresses:(NSArray *)toAddresses FromeAddresses:(NSString *)fromeAddresses withAmounts:(NSArray *)amounts withOmniAmounts:(NSString *)omniAmounts andChangeAddress:(NSString *)changeAddress dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error coin:(Coin)coin
{
    if (password && !self.hasPrivKey) {
        return nil;
    }
    ...
    NSArray *subOuts;
    if (coin != BTC) {
        subOuts = [[BTHDAccountAddressProvider instance] getPrevCanSplitOutsByHDAccount:self.hdAccountId coin:coin];
    } else {
        subOuts = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccount:self.hdAccountId];
    }
    for (BTOut *btOut in subOuts) {
        if ([btOut.outAddress isEqualToString:fromeAddresses]) {
            [outs addObject:btOut];
        }
    }
    BTTx *tx = [[BTTxBuilder instance] buildTxWithOutputs:outs toAddresses:toAddresses amounts:amounts changeAddress:changeAddress dynamicFeeBase:dynamicFeeBase andError:error];
    if (error && !tx) {
    ...
    long long omniAmount = [self numberValueString:omniAmounts decimal:@"8" isPositive:YES].longLongValue;
    NSString *omniHex = [NSString stringWithFormat:@"6f6d6e69%016x%016llx", [@"31" intValue], omniAmount];
    [omniScript appendData:BTCDataFromHex(omniHex)];
    [tx addOutputScript:omniScript.data amount:0];
...
- (BTTx *)newTxToAddresses:(NSArray *)toAddresses withAmounts:(NSArray *)amounts andChangeAddress:(NSString *)changeAddress pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error coin:(Coin)coin
{
    ...
    if (coin != BTC) {
        ...
        if (pathType == EXTERNAL_ROOT_PATH||pathType == INTERNAL_ROOT_PATH) {
            NSMutableArray *outsMut = [NSMutableArray new];
            NSArray *subOutsExternal = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccountId:self.hdAccountId pathType:EXTERNAL_ROOT_PATH];
            NSArray *subOutsInternal = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccountId:self.hdAccountId pathType:INTERNAL_ROOT_PATH];
            [outsMut addObjectsFromArray:subOutsExternal];
            [outsMut addObjectsFromArray:subOutsInternal];
            outs = outsMut;
        }else
        {
            NSMutableArray *outsMut = [NSMutableArray new];
            NSArray *subOutsExternal = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccountId:self.hdAccountId pathType:EXTERNAL_BIP49_PATH];
            NSArray *subOutsInternal = [[BTHDAccountAddressProvider instance] getUnspendOutByHDAccountId:self.hdAccountId pathType:INTERNAL_BIP49_PATH];
            [outsMut addObjectsFromArray:subOutsExternal];
            [outsMut addObjectsFromArray:subOutsInternal];
            outs = outsMut;
        }
...
- (BTTx *)newTxForVirtualTradingToAddresses:(NSArray *)toAddresses withAmounts:(NSArray *)amounts andChangeAddress:(NSString *)changeAddress pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error coin:(Coin)coin
{
...
- (BTTx *)newTxForVirtualTradingBc1ToAddresses:(NSArray *)toAddresses withAmounts:(NSArray *)amounts withOuts:(NSArray *)outs andChangeAddress:(NSString *)changeAddress pathType:(PathType)pathType dynamicFeeBase:(uint64_t)dynamicFeeBase password:(NSString *)password andError:(NSError **)error coin:(Coin)coin
{
...
- (BOOL)checkWithPassword:(NSString *)password {
    ...
    BOOL mnemonicSeefSafe = [[BTHDAccount seedFromMnemonic:self.mnemonicSeed withPassphrase:@""  btBip39:[BTBIP39 sharedInstance]] isEqualToData:hdCopy];
    [self wipeHDSeed];
    [self wipeMnemonicSeed];
...
+ (NSData *)seedFromMnemonic:(NSData *)mnemonicSeed withPassphrase:(NSString *)passphrase btBip39:(BTBIP39 *)bit39 {
    if (!bit39) {
        return [[BTBIP39 sharedInstance] toSeed:[[BTBIP39 sharedInstance] toMnemonic:mnemonicSeed] withPassphrase:passphrase];
    }
    return [bit39 toSeed:[bit39 toMnemonic:mnemonicSeed] withPassphrase:passphrase];
}
